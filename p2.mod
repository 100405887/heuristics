...
param AVAILABILITY{b in LOCATION,a in DISPONIBILITY};
...
s.t. share90 {m in LOCATION, a=0: AVAILABILITY[a,m]>0}: 
    sum{n in LOCATION} DISPLACEMENT[m,n] >= 0.9*AVAILABILITY[m,a];
    
# Scooters leaving an available location must be at least equal to total demand 
s.t. arriving: 
    sum{m in LOCATION,n in LOCATION:AVAILABILITY[n,0]>0} DISPLACEMENT[n,m] >= sum{n in LOCATION: AVAILABILITY[n,1]>0}AVAILABILITY[n,1];
    
# Scooters arriving to a demanding location must be at least the sum of its demand plus 
# scooters  leaving that location
s.t. demands {m in LOCATION: AVAILABILITY[m,0]>0}: 
    sum{n in LOCATION} DISPLACEMENT[n,m] >= AVAILABILITY[m,0] + sum{n in LOCATION}DISPLACEMENT[m,n];
    
# Sum of scooters leaving a location with no demand must be less or equal to 
# scooters arriving plus available scooters 
s.t. leaving {m in LOCATION: AVAILABILITY[m,0]>0}: 
    sum{n in LOCATION} DISPLACEMENT[m,n] <= AVAILABILITY[m,0] + sum{n in LOCATION}DISPLACEMENT[n,m];
end;
