...
def checkDepth(*args):
    ...
    for posi in args:
        ...
        if posi[1] != len(realmap) - 1:

            if (realmap[posi[1]+1][posi[0]]=='N' or realmap[posi[1]+1][posi[0]]=='E'):
...
def main():
   ...
   #variable created for each execution, which will differ according to the data received in the containers file
    ...
    for i in range(len(allconts)):

        for j in range(len(allconts)):

            if (contM[i][2]=="1" and contM[j][2] == "2"):

                problem.addConstraint(setPorts,[contM[i][0],contM[j][0]])
